...
def get_project_by_title(title):
    ...
    return Project(data[0], data[1])
        
def get_projects():
    c = cursor()
    projects = []
    for row in c.execute('SELECT * FROM projects'):
        projects.append(Project(row[0], row[1]))
    c.connection.close()
    return projects
...
def delete_project(project):
    c = cursor()
    with c.connection:
        c.execute('DELETE FROM projects WHERE title=? AND details=? AND total_target=? AND start_time=? AND end_time=?', (project.title, project.details, project.total_target, project.start_time, project.end_time))
    rows = c.rowcount
    c.connection.close()
    return rows
